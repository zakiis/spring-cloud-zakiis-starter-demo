...
public class SecurityDemoApplicationTests {
	...
	private String login() {
		...
		ResponseEntity<CommonResp<String>> responseEntity = restTemplate.exchange(url, HttpMethod.POST, new HttpEntity<>(loginRequest),
				new ParameterizedTypeReference<CommonResp<String>>() {});
	...
	private void userInfoQuery(String jwtToken) {
		...
		ResponseEntity<CommonResp<UserInfo>> responseEntity = restTemplate.exchange(url, HttpMethod.GET, httpEntity,
				new ParameterizedTypeReference<CommonResp<UserInfo>>() {});
		if (responseEntity.getStatusCode() == HttpStatus.OK) {
	...
	private void userInfoDelete(String jwtToken) {
		...
		HttpEntity<Object> httpEntity = new HttpEntity<>(null, httpHeaders);
		try {
			ResponseEntity<CommonResp<UserInfo>> responseEntity = restTemplate.exchange(url, HttpMethod.DELETE, httpEntity,
					new ParameterizedTypeReference<CommonResp<UserInfo>>() {});
			if (responseEntity.getStatusCode() == HttpStatus.OK) {
	...
	private Thread createUserModifyThread(String url, HttpEntity<Object> httpEntity) {
		return new Thread(() -> {
			ResponseEntity<CommonResp<Object>> responseEntity = restTemplate.exchange(url, HttpMethod.PUT, httpEntity,
					new ParameterizedTypeReference<CommonResp<Object>>() {});
	...
	private void testRateLimit() throws InterruptedException {
		...
		ResponseEntity<CommonResp<String>> responseEntity = restTemplate.exchange(url, HttpMethod.POST, null,
				new ParameterizedTypeReference<CommonResp<String>>() {});
